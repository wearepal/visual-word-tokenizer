...
class LossComputer:
    ...
    def reset_stats(self):
        ...
        self.avg_class_acc = torch.zeros(self.n_classes).cuda()
    ...
    def loss(self, yhat, y, group_idx=None):
        ...
        class_map = F.one_hot(y).T
        class_count = class_map.sum(axis=1)
        class_denom = class_count + (class_count == 0).float()  # avoid nans
        self.avg_class_acc = (class_map * (torch.argmax(yhat, 1) == y).float()).sum(axis=1) / class_denom
    ...
    def log_stats(self, path):
        ...
        with open(os.path.join(path, 'results.txt'), 'w') as f:
            ...
            print(f'Worst-group acc: {min(self.avg_group_acc):.3f}', file=f)
            print(f'Worst-class acc: {min(self.avg_class_acc):.3f}\n', file=f)
