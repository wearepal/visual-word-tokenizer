...
class ConfounderDataset(Dataset):
    ...
    def get_splits(self, splits, train_frac=1.0, shuffle=False):

        if shuffle:
            for n in np.unique(self.group_array):
                mask = (self.group_array == n)
                self.split_array[mask] = np.random.permutation(self.split_array[mask])
